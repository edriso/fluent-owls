...
Beyond the daily set, message the bot ([@FluentOwlsBot](https://t.me/FluentOwlsBot)) and it replies with a random item: **/listen** (any audio clip), **/quiz**, **/grammar**, **/phrase**, **/dialogue**, **/shadow**, **/monologue**, **/prompt**, **/pron**, **/vocab**, **/idiom**, **/story**, **/talk**. These are stateless (a random pick), so they need no database. Add an optional **CEFR level** to any of them to target it (for example `/story b1` or `/listen c1`), or send **/help** to see the whole menu. You can also **ask about a grammar point** — `/grammar present perfect`, or just type a topic like "second conditional" in a DM — and the bot finds the matching rule and sends it with spoken examples.
