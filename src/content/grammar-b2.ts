/**
 * B2 (upper-intermediate) grammar points: unreal conditionals, the passive,
 * reported speech, and the structures that make speech sound polished.
 */
import type { GrammarRule } from '../types';

export const b2Grammar: GrammarRule[] = [
  {
    id: 'b2-gr-001',
    rule: 'Second conditional (unreal present)',
    explanation: `If + past simple, then would + base verb. For imaginary or unlikely situations.`,
    examples: [
      'If I had time, I would travel.',
      'If I were you, I would wait.',
      'What would you do if you won?',
    ],
    note: `Use "were" for everyone here: "If I were you".`,
    audio: 'b2-gr-001.ogg',
  },
  {
    id: 'b2-gr-002',
    rule: 'Present perfect continuous',
    explanation: `Have/has been + -ing for an action that started in the past and is still going.`,
    examples: [
      'I have been working all day.',
      'It has been raining for hours.',
      'How long have you been waiting?',
    ],
    note: `It stresses the activity and its duration, not the result.`,
    audio: 'b2-gr-002.ogg',
  },
  {
    id: 'b2-gr-003',
    rule: 'The passive voice',
    explanation: `Be + past participle, when the action matters more than who did it.`,
    examples: [
      'The report was written by Sam.',
      'English is spoken here.',
      'The keys have been found.',
    ],
    note: `Add "by" only if you need to say who did it.`,
    audio: 'b2-gr-003.ogg',
  },
  {
    id: 'b2-gr-004',
    rule: 'Reported speech',
    explanation: `When you report what someone said, shift the tense one step back.`,
    examples: [
      'She said she was tired.',
      'He told me he would come.',
      'They said they had finished.',
    ],
    note: `"I am tired" becomes "She said she was tired".`,
    audio: 'b2-gr-004.ogg',
  },
  {
    id: 'b2-gr-005',
    rule: 'Relative clauses (who, which, that)',
    explanation: `Use "who" for people, "which" for things, and "that" for either.`,
    examples: [
      'The man who called is my boss.',
      'The book that I read was great.',
      'The car, which is new, broke down.',
    ],
    note: `Use commas only for extra, non-essential information.`,
    audio: 'b2-gr-005.ogg',
  },
  {
    id: 'b2-gr-006',
    rule: 'Modals of deduction',
    explanation: `Use "must" when you are sure, and "can't" when something is impossible.`,
    examples: ['She must be tired.', 'That can not be true.', 'He might be at home.'],
    note: `Use "might" or "could" when you are only guessing.`,
    audio: 'b2-gr-006.ogg',
  },
  {
    id: 'b2-gr-007',
    rule: 'wish for present regrets',
    explanation: `Use "wish" plus the past simple to talk about a present situation you want to be different.`,
    examples: [
      'I wish I had more time.',
      'I wish I were taller.',
      'She wishes she knew the answer.',
    ],
    note: `The past form here is about now, not the past: "I wish I had time" (but I do not).`,
    audio: 'b2-gr-007.ogg',
  },
  {
    id: 'b2-gr-008',
    rule: 'so and such',
    explanation: `Use "so" before an adjective alone, and "such" before (a) plus a noun.`,
    examples: [
      'It was so cold.',
      'It was such a cold day.',
      'She is so kind. She is such a kind person.',
    ],
    note: `so + adjective; such + (a/an) + (adjective) + noun.`,
    audio: 'b2-gr-008.ogg',
  },
  {
    id: 'b2-gr-009',
    rule: `Past perfect (had + past participle)`,
    explanation: `Use it for an action that finished before another past action.`,
    examples: [
      'By the time we arrived, the film had started.',
      'She had left before I called.',
      'They had never seen snow before that trip.',
    ],
    note: `It marks the earlier of two past events.`,
    audio: 'b2-gr-009.ogg',
  },
  {
    id: 'b2-gr-010',
    rule: `Future continuous (will be + -ing)`,
    explanation: `Use it for an action that will be in progress at a future time.`,
    examples: [
      "This time tomorrow I'll be flying to Rome.",
      "Don't call at 8; I'll be driving.",
      "She'll be working late tonight.",
    ],
    note: `It stresses being in the middle of something, not just that it happens.`,
    audio: 'b2-gr-010.ogg',
  },
  {
    id: 'b2-gr-011',
    rule: `Causative: have something done`,
    explanation: `Use "have/get + object + past participle" when someone else does the job for you.`,
    examples: [
      'I had my car repaired.',
      'She got her hair cut.',
      'We are having the kitchen painted.',
    ],
    note: `It is not you doing it: "I had my car repaired" (a mechanic did it).`,
    audio: 'b2-gr-011.ogg',
  },
  {
    id: 'b2-gr-012',
    rule: `Gerunds after prepositions`,
    explanation: `After a preposition, a verb takes the -ing form.`,
    examples: [
      'She is good at swimming.',
      "I'm interested in learning Spanish.",
      'Thank you for helping.',
    ],
    note: `Preposition + verb-ing, always: "at swimming", not "at swim".`,
    audio: 'b2-gr-012.ogg',
  },
  {
    id: 'b2-gr-013',
    rule: `Defining vs non-defining relative clauses`,
    explanation: `A defining clause is essential (no commas); a non-defining clause adds extra info (commas).`,
    examples: [
      'The book that I bought was cheap.',
      'My brother, who lives in Spain, is a doctor.',
      'The car, which is new, broke down.',
    ],
    note: `Commas mean the information is extra, not needed to identify.`,
    audio: 'b2-gr-013.ogg',
  },
  {
    id: 'b2-gr-014',
    rule: `although and however`,
    explanation: `"Although" joins two clauses in one sentence; "however" links two sentences.`,
    examples: [
      'Although it rained, we walked.',
      'It was late. However, we kept going.',
      'Although she is young, she is wise.',
    ],
    note: `"however" usually starts a new sentence and takes a comma.`,
    audio: 'b2-gr-014.ogg',
  },
  {
    id: 'b2-gr-015',
    rule: `used to vs be used to`,
    explanation: `"used to + base verb" is a past habit; "be used to + -ing/noun" means familiar with.`,
    examples: ['I used to smoke.', "I'm used to the noise now.", 'She is used to working late.'],
    note: `used to do (past habit) vs be used to doing (it feels normal).`,
    audio: 'b2-gr-015.ogg',
  },
  {
    id: 'b2-gr-016',
    rule: `Zero conditional (general truths)`,
    explanation: `If + present simple, present simple. For things that are always true.`,
    examples: [
      'If you heat water, it boils.',
      'If I am tired, I go to bed early.',
      'Plants die if you do not water them.',
    ],
    note: `Both parts use the present; "if" here means "whenever".`,
    audio: 'b2-gr-016.ogg',
  },
  {
    id: 'b2-gr-017',
    rule: `Reported questions`,
    explanation: `When you report a question, use statement word order and no question mark.`,
    examples: [
      'She asked where I lived.',
      'He asked if I was ready.',
      'They asked what time it started.',
    ],
    note: `Use "if" or "whether" for yes/no questions: "asked if I was ready".`,
    audio: 'b2-gr-017.ogg',
  },
  {
    id: 'b2-gr-018',
    rule: `Verbs that change meaning: stop, remember, try`,
    explanation: `Some verbs mean something different with -ing or with "to".`,
    examples: ['I stopped smoking.', 'I stopped to smoke.', 'Remember to lock the door.'],
    note: `stop doing = quit it; stop to do = pause in order to do it.`,
    audio: 'b2-gr-018.ogg',
  },
  {
    id: 'b2-gr-019',
    rule: `wish + would for annoyance`,
    explanation: `Use "wish + would" to complain about an annoying habit you want someone to change.`,
    examples: [
      'I wish you would listen.',
      'I wish it would stop raining.',
      'She wishes he would call.',
    ],
    note: `Use it for other people's habits, not your own situation.`,
    audio: 'b2-gr-019.ogg',
  },
  {
    id: 'b2-gr-020',
    rule: `Relative clauses with whose, where, when`,
    explanation: `Use "whose" for possession, "where" for places, and "when" for times.`,
    examples: [
      "That's the man whose car was stolen.",
      'This is the town where I grew up.',
      'I remember the day when we met.',
    ],
    note: `"whose" means belonging to; "where" means in or at which.`,
    audio: 'b2-gr-020.ogg',
  },
  {
    id: 'b2-gr-021',
    rule: `Contrast and reason: in spite of, due to`,
    explanation: `Use "in spite of" plus a noun or -ing for contrast, and "due to" for a cause.`,
    examples: [
      'We went out in spite of the rain.',
      'In spite of being tired, she finished.',
      'The delay was due to traffic.',
    ],
    note: `"in spite of" equals "despite"; both take a noun or -ing, not a clause.`,
    audio: 'b2-gr-021.ogg',
  },
  {
    id: 'b2-gr-022',
    rule: `Passive with two objects`,
    explanation: `When a verb has two objects, either one can become the subject of the passive.`,
    examples: ['I was given a present.', 'A present was given to me.', 'She was offered the job.'],
    note: `Common with give, offer, send, tell; the person often comes first.`,
    audio: 'b2-gr-022.ogg',
  },
];
