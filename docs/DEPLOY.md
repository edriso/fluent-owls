...
- **A voice post did not arrive** (`Failed to post shadowing voice` or `Failed to post role-play dialogue`, "is the audio generated?"). That one `.ogg` is missing from the clip folder. Run `pnpm generate-audio` to create it and rsync it to the host's clip folder (`/opt/bots/data/fluent-owls/audio`), or run `pnpm audit-speaking --require-audio` to find every gap. The other posts are unaffected.
