...
│   │   ├── audio-path.ts    Where the .ogg clips live and how to find one.
│   │   ├── audio/           OGG/Opus clips for every audio kind. Git-ignored (ElevenLabs output, see NOTICE); only README.md is tracked. Generated by scripts/generate-audio.ts; in prod a read-only bind mount.
...
- **Audio is pre-generated, never made at runtime, and kept OUT of git.** The `.ogg` files are created by `scripts/generate-audio.ts` (ElevenLabs TTS, then ffmpeg to OGG/Opus) into `src/content/audio/`. They are ElevenLabs output under the owner's commercial license (not redistributable, see `NOTICE`), so they are git-ignored and not in the repo: a contributor generates their own with `pnpm generate-audio`. In production they are durable host data supplied by a read-only bind mount (see `docs/DEPLOY.md`), not baked into the image. The running bot only reads them, so production needs no TTS key, has no audio cost, and gains no new failure mode. This is the same "runs for years untouched" ethos as the no-database rule. A clip's `text` is both the transcript shown in the caption and the script the audio is generated from, so the two cannot drift.
...
- **Vocabulary is its own content type.** A `VocabularyEntry` (`src/content/vocabulary-*.ts`, registry `vocabulary.ts`) teaches one word in depth: a `pronunciation` guide (IPA in slashes plus a plain respelling, e.g. `/kənˈsɪdərət/ (kuhn-SID-uh-rit)`, shown small under the word), a plain `meaning`, 2 to 3 `examples`, and a usage `note` (collocation, synonym, or common mistake). It posts as a voice message with an HTML caption, via `postVocabulary`. The audio reads ONLY the example sentences, NOT the lone word: a TTS voice often mispronounces a word out of context (the "-ate" adjectives say a strong "-ate" instead of the weak "-it"), so the learner hears the word correctly inside the examples and reads it, with its pronunciation, in the caption. Same shape as the other voice types; the audit enforces a unique word per entry and the `/IPA/ (respelling)` pronunciation format.
...
- The vocabulary banks: unique `<level>-vc-` ids, a unique word per entry, a `pronunciation` guide in `/IPA/ (respelling)` form within its limit, word/meaning/note within limits, 2 to 3 examples within limits, a rendered HTML caption that contains both the word and the pronunciation, an audio name of `<id>.ogg`, and no em-dashes.
...
- `CONTENT_COMMANDS`: the on-demand command registry has the expected commands in order, unique names/aliases (incl. the /pron alias), the right /listen subset (every type but quiz), grammar as the one custom handler, and the fields /help and the menu need.
- `channelUrlFrom`: the `/start` DM link (port resolution moved to the kernel).
- `audio-path`: `countAudioClips` counts the `.ogg` files on disk (zero in a clean checkout, since clips are git-ignored) and `audioPathFor` resolves under `AUDIO_DIR`. The test checks the contract (path resolution, a count that matches disk and reacts to a probe clip, never throwing), NOT that any clip is present. This backs the boot-time check in `index.ts` that logs loudly when the audio is missing (the bind mount absent in prod, or clips never generated in dev) (the bug that once left every voice post silent). Per-item audio parity stays the opt-in `pnpm audit-speaking --require-audio` gate, since content may be valid before its clips exist.

No test needs a real bot token, a specific audio file, or a database; `vitest.config.ts` injects placeholders (no `DATABASE_URL`, so the tutor stays off in tests) and the posters are mocked. `audio-path.test.ts` only reads (and writes a self-cleaning probe into) the clip directory, never depending on any real clip being present. `pnpm audit-all` (quizzes + speaking) is a separate, network-free data check.
...
- **All audio must be generated and present on disk (but NOT committed)**: the voice posters (`postVoice`, `postDialogue`, `postGrammar`, `postMonologue`) read `src/content/audio/<id>.ogg` from disk and upload via `sendVoice`. A missing file is caught and logged ("is the audio generated?") and the rest of the batch still posts. Run `pnpm generate-audio` (needs `ELEVENLABS_API_KEY` and `ffmpeg`); the `.ogg` are git-ignored, so in production you `rsync` them to the host's clip folder (`/opt/bots/data/fluent-owls/audio`, bind-mounted read-only), never commit them. `pnpm audit-speaking --require-audio` fails on any gap across all four audio kinds, for a pre-deploy gate. Dialogues (one call per turn) and grammar (one call per example) make several calls and concatenate, so they use more credits per item than a single shadowing clip.
- **On-demand commands reply to the asker, not the channel**: every poster takes an optional `chatId` (default `config.channelChatId`). The on-demand handlers pass `ctx.chat.id`, so a DM request gets a private reply. Random picks use `Math.random` (fine in the bot runtime). These commands are generated from the `CONTENT_COMMANDS` registry in `src/content-commands.ts`, which is also what `/listen`, `/help`, and the command menu read, so adding a type or renaming a command is a one-line change there (a test in `tests/content-commands.test.ts` locks its shape). `/grammar` is the one custom handler (it also does topic search).
